# Defect Detection

Computer vision based industrial defect detection system.

## Goal

Detect product defects from images using machine learning.

## Tech Stack

- Python
- OpenCV
- PyTorch
- Docker

## Project Status

Environment setup


## Steps I did

- I structured project folder in standard convention then _git init_
```text
root/
    data/
    models/
    notebooks/
    src/
    tests/
```
- I made branch 'feature/docker-setup' to set up Docker development workflow with docker compose (struggle: docker dependencies)
- Choose Kaggle dataset - Surface Crack Detection (https://www.kaggle.com/datasets/arunrk7/surface-crack-detection)
- I noticed it is Binary Classification problem.
